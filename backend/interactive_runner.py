...
Runs a student's already-compiled program live so prompts appear before reads
(``input()`` / ``scanf`` / ``cin`` / ``Scanner``), just like a real terminal.
Works for any language — the caller passes a prepared ``run_argv`` (e.g.
``[python, -u, solution.py]`` or ``[./sol]`` or ``[java, -cp, dir, Main]``).
...
plain pipe. Output still streams live; the only difference is there's no terminal
echo (the frontend echoes typed input itself when it sees the 'pipe' mode).
"""
import os
import subprocess
import sys
...
# Same interpreter that runs the backend (used only by legacy string callers).
PYTHON = sys.executable or "python3"


def _cmd(run_argv):
    """Accept a prepared argv list, or a bare script path (legacy → python -u)."""
    if isinstance(run_argv, str):
        return [PYTHON, "-u", run_argv]
