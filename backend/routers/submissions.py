...
def memory_check(payload: VisualizeRequest, _user: models.User = Depends(get_current_user)):
    """Static "Code Check": pyflakes (Python) or compiler warnings (C/C++/Java) plus one run."""
    return memcheck(payload.code, payload.language, payload.custom_input)
...
def visualize(payload: VisualizeRequest, _user: models.User = Depends(get_current_user)):
    ...
    sys.settrace, for the Code Visualizer. PYTHON ONLY — the tracer relies on
    CPython internals; for other languages we return a clear note so the UI can
    hide the visualizer.
    """
    from code_runner import normalize_language
    if normalize_language(payload.language) != "python":
        return {"error": "The Code Visualizer is available for Python only.",
                "unsupported": True, "steps": []}
...
def run_code(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        run_argv, compile_msg = compile_code(payload.code, payload.language, tmpdir)
        if run_argv is None:
            return {"status": "Compilation Error", "output": compile_msg, "time_ms": 0}
        result = run_once(run_argv, payload.custom_input, time_limit=8.0)
        return {
...
def run_samples(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for idx, case in enumerate(payload.cases):
            run = run_once(run_argv, case.input_data, time_limit=8.0)
...
async def run_interactive(ws: WebSocket):
    ...
    try:
        ...
        run_argv, compile_error = compile_code(code, language, tmpdir, force_unbuffered=True)
        if run_argv is None:
        ...
        try:
            session, mode, note = make_session(run_argv)
...
def submit_code(
    ...
    # Enforce the problem's allowed languages (falls back to all four).
    allowed = getattr(problem, "allowed_languages", None) or list(models.LANGUAGES)
    language = payload.language or "python"
    if language not in allowed:
        raise HTTPException(400, f"Language '{language}' is not allowed for this problem")

    time_limit = float(getattr(problem, "time_limit_sec", None) or 5)
    verdict = judge_submission(payload.code, language, test_cases, time_limit)
