...
def backup(format: str = "json", db: Session = Depends(get_db), _admin=Depends(get_admin_user)):
    ...
    if format == "xlsx":
        ...
        return Response(
            ...
            headers={"Content-Disposition": f"attachment; filename=codearena_backup_{stamp}.xlsx"},
        )
    ...
    return Response(
        ...
        headers={"Content-Disposition": f"attachment; filename=codearena_backup_{stamp}.json"},
    )
...
def backup_files(_admin=Depends(get_admin_user)):
    ...
    return Response(
        ...
        headers={"Content-Disposition": f"attachment; filename=codearena_files_{stamp}.zip"},
    )
