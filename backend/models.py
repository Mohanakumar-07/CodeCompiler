...
from sqlalchemy import (
    JSON, BigInteger, Boolean, Column, DateTime, Enum, Float,
...
# Languages supported by the editor + judge.
...
class Problem(Base):
    ...
    starter_code = Column(Text)           # scaffolded template (Python / primary language)
    starter_code_map = Column(JSON)       # optional per-language starters {python, c, cpp, java}
    allowed_languages = Column(JSON)      # optional list; null/empty → all four allowed
    time_limit_sec = Column(Integer, default=5)   # per-run wall-clock limit for the judge
