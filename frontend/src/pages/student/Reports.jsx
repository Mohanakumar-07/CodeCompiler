...
import { Eye, CheckCircle, XCircle, Clock, Code2, Download, Award, Search, MessageSquare, AlertTriangle } from 'lucide-react'
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={() => downloadFile(`/reports/export${mode ? `?mode=${mode}` : ''}`, 'codearena_transcript.xlsx').catch(() => toast.error('Export failed'))}
          >
    ...
    <div className="space-y-5">
      ...
      {r.tab_switches > 0 && (
        <div className="p-3 rounded-lg border border-line text-sm" style={{ background: 'color-mix(in srgb, var(--warn) 12%, transparent)', color: 'var(--warn)' }}>
          <AlertTriangle size={14} className="inline-block flex-shrink-0" /> Tab switches recorded: {r.tab_switches}
        </div>
      )}
